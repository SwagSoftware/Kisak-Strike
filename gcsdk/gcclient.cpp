...
public:
	...
	virtual bool BYieldingRunGCJob( GCSDK::IMsgNetPacket *pNetPacket )
	{
		...
		m_pGCClient->NotifySOCacheUnsubscribed( CSteamID(uint64(msg.Body().owner_soid().id())) );
...
public:
	...
	virtual bool BYieldingRunGCJob( GCSDK::IMsgNetPacket *pNetPacket )
	{
		...
		if ( pSOCache == NULL || !pSOCache->BIsInitialized() || pSOCache->GetVersion() != msg.Body().version() )
		{
			...
			msg_response.Body().set_allocated_owner_soid( msg.Body().mutable_owner_soid() );
