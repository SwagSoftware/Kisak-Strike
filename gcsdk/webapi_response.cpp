...
void CWebAPIValues::SetName( const char * pchName ) 
{
	...
	else
	{
		...
		while ( pchName[unLen] != 0 )
		{
			...
			{
				AssertMsg( false, "Shouldn't use any of '\"&<>: in CWebAPIValues node names, you used %s" );
				break;
			}
			++unLen;
		}
	...
	case k_EWebAPIValueType_String:
		{
			...
			AssertMsg1( uiVal < INT_MAX, "GetInt32Value called on node with %u, which is too big to fit in an Int32", uiVal );
			return (int32)uiVal;
		}
	}

	AssertMsg1( false, "Shouldn't call CWebAPIValues::GetInt32Value unless value type is int32, uint32, uint64, or string. %d does not.", m_eValueType );
	return 0;
}
	...
	case k_EWebAPIValueType_String:
		{
			...
			AssertMsg1( uiVal <= UINT_MAX, "GetUInt32Value called on node with %llu, which is too big to fit in an UInt32", uiVal );
			return (uint32)uiVal;
		}
	}

	AssertMsg1( false, "Shouldn't call CWebAPIValues::GetUInt32Value unless value type is uint32, uint64, or string. %d does not", m_eValueType );
	return 0;
}
