...
#if defined(VERSION_SAFE_STEAM_API_INTERFACES) && !defined(_PS3)
...
#else
#define g_pSteamClientGameServer 1
#endif
...
inline bool CSteamGameServerAPIContext::Init()
{
	...
	m_pSteamGameServerUtils = ::SteamUtils();
	if ( !m_pSteamGameServerUtils )
		return false;



	m_pSteamGameServerNetworking = ::SteamNetworking();
	if ( !m_pSteamGameServerNetworking )
		return false;

	m_pSteamGameServerStats = NULL; // ::SteamGameServerStats(); -- unsupported on PS3
